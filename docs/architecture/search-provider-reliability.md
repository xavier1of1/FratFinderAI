...
This document preserves the summarized findings from earlier local stress and queue-throughput artifacts. Those raw one-off JSONL/log artifacts were intentionally removed from the public repository during the release cleanup because they were generated local run output rather than durable source documentation.
...
`provider_health_snapshots.healthy` is **not provider-specific health**. In `services/crawler/src/fratfinder_crawler/orchestration/request_graph.py`, each provider snapshot row is inserted with:
...
- `search_preflight()` in `services/crawler/src/fratfinder_crawler/pipeline.py` aggregates only:
  - `attempts`
  - `successes`
  - `unavailable`
  - `request_error`
...
From the removed local `DEEP_ANALYSIS_2026-04-09.md` stress artifact summary:
...
From the removed local queue-throughput proof artifact, aggregated across the recorded provider windows:
...
From `services/crawler/src/fratfinder_crawler/search/client.py`:
